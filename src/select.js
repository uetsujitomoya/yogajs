	...
	for(m=1;m<keitaisokaiseki.length;m=m+2){
		...
		for(i=0;i<keitaisokaiseki[m].length;++i){
			...
			if(RGB[m][i][0]+RGB[m][i][1]+RGB[m][i][2]>=2){
				target.innerHTML += "<div id=\"b" + c + "\">"+ bun[m][i] + "</div><div id=\"r" + c + "\"><font size=1 color=purple>（" + (m+1) + " " + hatsugen[m] + "）</font><br><label><input type=radio name=\"r" + c + "\" value=0>どれにも含まない</label></div><br>";

			}else{
				target.innerHTML += "<div id=\"b" + c + "\"><font size=1 color=gray>"+ bun[m][i] + "</div><div id=\"r" + c + "\" class=\"hide\"><font size=1 color=purple>（" + (m+1) + " " + hatsugen[m] + "）</font><br><label><input type=radio name=\"r" + c + "\" value=0>どれにも含まない</label></div></font><br>";
			}
